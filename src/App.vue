  ...
  <div class="min-h-screen bg-white text-black">
    ...
    <header class="fixed top-0 left-0 w-full bg-white shadow-md p-4 flex items-center justify-between z-10">
      <div class="flex items-center">
        <img :src="logoUrl || 'https://via.placeholder.com/40x40?text=Logo'" alt="Logo" class="w-10 h-10 mr-2">
      ...
      <div class="flex items-center space-x-4">
        ...
        <button @click="$refs.logoInput.click()" class="px-2 py-1 bg-orange-500 text-white rounded-full font-semibold hover:bg-orange-600 transition">Change Logo</button>
...
import { storage, db } from './firebase' // Sửa đường dẫn từ ../firebase thành ./firebase
  ...
  methods: {
    ...
    async handleLogoUpload(event) {
      ...
      try {
        ...
        const storageRef = ref(storage, `logos/logo-${Date.now()}.${file.name.split('.').pop()}`)
